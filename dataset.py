...
class WN18RR(InMemoryDataset):
    edge2id = {
        ...
        "_instance_hypernym": 3,
        "_also_see": 4,
        "_member_meronym": 5,
        "_synset_domain_topic_of": 6,
        "_has_part": 7,
    ...
    def process(self):
        ...
        edge_index = torch.tensor(
            relations[["id", "related_id"]].map(entity_mapping.get).to_numpy().T,
            dtype=torch.long,
        )
        ...
        with torch.no_grad():
            for batch_idx, (token_id, mask, type_id) in enumerate(token_loader):
                if batch_idx % 100 * batch_size and batch_idx > 0:
                    torch.save(
                        encodings, os.path.join(self.root, "partial_encodings.pt")
                    )
                    if self._verbose_processing:
                        print(f"\tBatch {batch_idx}/{num_batches}")
                batch_encoding = encoder(
                    token_id.to(device), mask.to(device), type_id.to(device)
                ).last_hidden_state[:, 0, :]
    ...
    def _split_map(self, split):
        if split == "train":
            ...
            raise ValueError(
                'Invalid split name. Choose from "train", "validation" or "test"'
            )
...
class RelationsDS(InMemoryDataset):
    edge2id = {"Hypernyms": 1, "Holonyms": 2}

    def __init__(
        self, root, tokenizer=None, transform=None, pre_transform=None, pre_filter=None
    ):
    ...
    def process_csv(self):
        ...
        all_definitions = pd.concat(
            [df["Definição_Synset"], df["Definição_Relacionada"]]
        )

        entities = pd.DataFrame(
            {"ID": all_ids, "Definição": all_definitions}
        ).drop_duplicates()
    ...
    def _create_splits(self, data, device):
        src, tar, neg = structured_negative_sampling(
            data.edge_index, contains_neg_self_loops=False
        )
        ...
        train_mask, val_mask = train_test_split(
            train_mask, test_size=0.2, stratify=labels[train_mask]
        )
...
def split_data_stratified(g, num_nodes, add_self_loops=True):
    ...
    src, tar, neg = structured_negative_sampling(
        g.edge_index, contains_neg_self_loops=False
    )
    ...
    if add_self_loops:
        ...
        edge_index_i = torch.cat(
            [edge_index_i, torch.arange(num_nodes, dtype=torch.int64, device=device)]
        )
        edge_index_j = torch.cat(
            [edge_index_j, torch.arange(num_nodes, dtype=torch.int64, device=device)]
        )
        edge_index = torch.stack([edge_index_i, edge_index_j])

        edge_attr = g.edge_attr[train_mask]
        edge_attr = torch.cat(
            [edge_attr, torch.zeros(num_nodes, dtype=torch.long, device=device)]
        )
    else:
...
if __name__ == "__main__":
    train = WN18RR(
        "data/wn18rr", split="train", encoder_batch_size=1, verbose_processing=True
    )
